...
_conv_stride = np.array([2,1,1,1,1]) # Strides (one value for each layer)
_filtergroup_yn = np.array([0,1,0,1,1], dtype=bool) # Filtergroup : For some layer 2, 4 and 5 split weights
_bnorm_yn = np.array([1,1,1,1,0], dtype=bool) # Batch norm everywhere except on last layer
_relu_yn = np.array([1,1,1,1,0], dtype=bool) # ReLU everywhere except on last layer
